<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('komisi.create') }}" style="background-color: #1e293b; color: white;" class="flex items-center gap-2 bg-slate-800 hover:bg-slate-900 active:bg-slate-950 text-white px-5 py-2.5 rounded-xl font-bold shadow-md hover:shadow-lg transition-all duration-200 border border-slate-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow duration-300">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <h3 class="text-lg font-bold text-gray-900">{{ $komisi->nama }}</h3>
                                <p class="text-sm text-gray-600">{{ $komisi->keterangan }}</p>
                            </div>
                            <div class="flex gap-2">
                                <!-- Edit Button -->
                                <a href="{{ route('komisi.edit', $komisi) }}" class="text-blue-600 hover:text-blue-900 border border-blue-200 rounded-lg p-1 hover:bg-blue-50 transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <!-- Delete Button -->
                                <button @click="confirmingDeletion = true; deletingUrl = '{{ route('komisi.destroy', $komisi) }}'" class="text-red-600 hover:text-red-900 border border-red-200 rounded-lg p-1 hover:bg-red-50 transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 2 0 00-1-1h-4a1 2 0 00-1 1v3M4 7h16"></path></svg>
                                </button>
                            </div>
                        </div>
                        
                        <div class="flex gap-4 text-sm text-gray-500 border-t pt-4">
                            <div>
                                <span class="block font-medium text-gray-900">{{ $komisi->anggotas->count() }}</span>
                                Anggota
                            </div>
                            <div>
                                <span class="block font-medium text-gray-900">{{ $komisi->pendampings->count() }}</span>
                                Pendamping
                            </div>
                        </div>
                    </div>
